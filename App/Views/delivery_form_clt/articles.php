		...
		<div class="col-sm-6 col-xs-12">
			<div class="box-infos-search">
				...
				<div class="box-infos">
					...
					<h3>الرقم: <?= $delivery_form_clt->num ?></h3>
					<p>التاريخ: <?= $delivery_form_clt->dt ?></p>
					<p>الموضوع: <?= $delivery_form_clt->subject ?></p>
					<p>ملاحظات: <?= $delivery_form_clt->discr ?></p>
				</div>
			</div>
		</div>
		<div class="col-sm-6 col-xs-12">
	...
	<section class="content-header">
		...
		<ul class="header-btns">
			...
			<li>
				<a href="#" class="btn btn-warning" onclick="loadPurchase_ordersClt(event);" title=" استيراد طلب منتجاتن">
		...
		<div class="table-responsive">
			...
			foreach($delivery_form_clt_arts as $delivery_form_clt_art): ?>
				...
				<?php if(isset($_SESSION['user']) && $_SESSION['user']->aed_articles): ?>
					<a href="<?= App::$path ?>delivery_form_clt/editart/<?= $delivery_form_clt_art->delivery_form_clt_id ?>/<?= $delivery_form_clt_art->id ?>" class="btn btn-warning btn-xs">تعديل</a>
						<a href="#" class="btn btn-danger btn-xs" delivery_form_art_id="<?= $delivery_form_clt_art->id ?>" onclick="deleteElementArt(this, event);">حذف</a>
