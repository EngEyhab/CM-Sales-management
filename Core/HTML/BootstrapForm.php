...
class BootstrapForm extends Form{
    ...
    public function input($name, $label, $attributes = []){
        $label = '<label>'.$label.'</label>';
        $input =  '<input name="'.$name.'" '.$this->extractAttrs($attributes).' value="'.$this->getValue($name).'"  class="form-control">';
    ...
    public function submit($name, $caption, $attributes = []){
        return  '<button type="submit" name="'.$name.'" '.$this->extractAttrs($attributes).' class="btn btn-success btn-lg btn-block">'.$caption.'</button>';
    }
}